...
file_users=r"C:\Users\user\OneDrive\Documents\MY.VSCODES\MCQ-THE_BEATALLS\users.json"
...
def signin_signup(users):
   ...
   # if the user exists then print their history
    if id in users:
        print(f"Good to see you again, {id}!")
        print(f"Below you can find your playing history:") 
        for x in users[id]["history"] : 
            print(f"Score: {x["score"]}, \t Date: {x["date"]}, \t Time: {x["time"]}")
    # else, sign them up and add their id to the users.json          
    else:
        print(f"Creating an account for {id}... ")
        users[id]={"history" : []}
